...
// wall, at half the enclosure's height -- positioned at electronics-bay
...
module sensor_hole() {
    // Drilled through the right wall, centered in the electronics bay's
    // depth (not the panel's) so the panel isn't sitting right in front
    // of the hole. rotate([0,90,0]) maps the cylinder's local +z (its
    // extrusion axis) onto global +x, so it must start 1mm before the
    // wall's inner face to fully clear both faces after the wall + 2mm
    // cylinder length.
    translate([outer_w - wall - 1, outer_h / 2, wall + electronics_clearance / 2])
        rotate([0, 90, 0])
